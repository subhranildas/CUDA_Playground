...
static int aes_test_ecb_key128_singleblock()
{
	...
	if (enc_ok && dec_ok)
	{
		printf("[CPU] AES-128 single-block round-trip PASSED\n");
	}
	else
	{
		overall_ok = false;
		printf("[CPU] AES-128 single-block test FAILED\n");
		if (!enc_ok)
		{
...
static int aes_test_ecb_key128_multiblock()
{
	...
	if (memcmp(multi_plain, multi_decrypted, len) == 0)
		printf("[CPU] AES-128 multi-block round-trip PASSED\n");
	else
	{
		overall_ok = false;
		printf("[CPU] AES-128 multi-block round-trip FAILED\n");
	}
...
static int aes_test_ecb_key192_multiblock()
{
	...
	if (memcmp(multi_plain, multi_decrypted, len) == 0)
		printf("[CPU] AES-192 multi-block round-trip PASSED\n");
	else
	{
		overall_ok = false;
		printf("[CPU] AES-192 multi-block round-trip FAILED\n");
	}
...
static int aes_test_ecb_key256_multiblock()
{
	...
	if (memcmp(multi_plain, multi_decrypted, len) == 0)
		printf("[CPU] AES-256 multi-block round-trip PASSED\n");
	else
	{
		overall_ok = false;
		printf("[CPU] AES-256 multi-block round-trip FAILED\n");
	}
